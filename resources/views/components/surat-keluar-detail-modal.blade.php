...
<div
    ...
    x-on:open-surat-keluar-modal.window="if ($event.detail.name === '{{ $name }}') { open = true; surat = $event.detail.surat; }"
...
>
    ...
    <div class="relative bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto" x-show="open" x-transition.scale>
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Detail Surat Keluar</h3>
        ...
        <div class="p-6" x-show="surat">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-900 bg-gray-50 p-2 rounded font-medium" x-text="surat?.surat_keluar_nomor || '-'"></p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Surat</label>
                    <p class="text-sm text-gray-900 bg-gray-50 p-2 rounded" x-text="surat?.surat_keluar_tanggal_formatted || '-'"></p>
                </div>
                <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Unit Kerja Tujuan</label>
                    <p class="text-sm text-gray-900 bg-gray-50 p-2 rounded" x-text="surat?.unit_kerja_tujuan?.unit_kerja || '-'"></p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Bagian/Seksi Tujuan</label>
                    <p class="text-sm text-gray-900 bg-gray-50 p-2 rounded" x-text="surat?.bagian_seksi_tujuan?.bagian_seksi || '-'"></p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Bagian/Seksi Pembuat</label>
                    <p class="text-sm text-gray-900 bg-gray-50 p-2 rounded" x-text="surat?.bagian_seksi_pembuat?.bagian_seksi || '-'"></p>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 pt-4">
                <div class="text-xs text-gray-500 mb-2">
                    <strong>Informasi Pembuat:</strong>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        <span class="text-gray-600">Dibuat oleh:</span>
                        <span class="font-medium text-gray-900" x-text="surat?.creator?.nama || 'Unknown'"></span>
                    </div>
                    <div>
                        <span class="text-gray-600">Tanggal dibuat:</span>
                        <span class="text-gray-900" x-text="surat?.created_at_formatted || '-'"></span>
                    </div>
                </div>
            </div>
        </div>
